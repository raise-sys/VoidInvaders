window.voidInvaders = (() => {
    ...
    function syncPauseButton() {
        ...
        button.setAttribute('aria-label', paused ? 'ゲームを再開' : '一時停止');
        button.title = paused ? 'ゲームを再開' : '一時停止';
    }
    ...
    function update(dt) {
        ...
        let interval = Math.max(.07, .62 * (alive.length / 55)) / (1 + (game.wave - 1) * .08);
        if (alive.length === 1) interval *= .5;
        game.marchTimer += dt;
        ...
        // The shields may be overrun; the game ends only when an invader reaches the cannon line.
        if (alive.some(e => e.y + e.h >= game.player.y)) loseLife(true);
    ...
    function onKey(e,down){
        ...
        if(down&&e.code==='KeyP'&&(game.state==='playing'||game.state==='paused'))togglePause();
    }
    ...
    return {
        init(id){
            ...
            if(pauseButton)pauseButton.addEventListener('pointerdown',e=>{e.preventDefault();togglePause();});
            document.querySelector('.control-deck')?.addEventListener('contextmenu',e=>e.preventDefault());
